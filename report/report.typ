...
#align(center)[
  ...
  #v(2pt)
  #text(12pt)[Monetizing Fresh Satellite Substrate as Enforcement-Grade
  Dossiers for Illegal Farmland Factories]
  #v(4pt)
  #text(10pt, style: "italic")[Big Data Systems — Final Project · National Taiwan University · Spring 2026]
]
...
#box(inset: 8pt, radius: 5pt, fill: rgb("#fbfaf4"), stroke: 0.5pt + rgb("#d9d6c8"), width: 100%)[
  *Thesis.* Raw, public, easy data has no durable value: in a competitive market
  ...
  sells the agent's output artifact — a per-parcel enforcement dossier — not the
  data underneath.* The big-data pipeline (multi-temporal satellite imagery fused
  with cadastral/zoning data and statute retrieval) exists to feed the agent a
  substrate fresh and well-structured enough that its dossier beats anything the
  customer could get from a generic chatbot. The exploitable asymmetry: *government
  has authority but no eyes.* It can lawfully cut water/electricity and demolish
  illegal farmland structures, but it does not continuously watch the farmland, so
  it learns of violations late. The technical edge is *collapsing the observation
  lag* — and there is no luck in it: either the pipeline sees the new structure or
  it doesn't.
]
...
— 地球公民基金會 (Citizens of the Earth Taiwan) and the g0v community behind
...
drafting, is exactly the cognitive labour AgriSentinel automates — we do not
invent demand, we replace a crowdsourcing game and hand-written dossiers with a
pipeline, riding the NGO→government relationship as the cold-start wedge.

*Why us over the status quo.* The status quo is anonymous citizen reports + a
volunteer game + manual paperwork: slow, unsystematic, and unscalable across the
...
of careful review per image across tens of thousands of parcels per cycle; and (b)
...
†page-capped at 100 — true counts are higher. Government bodies already procure
...
≈3 weeks to deploy — proof the exact pipeline ships commercially.

*5 — The defensible claim.* _N_ flagged parcels at precision _P_ (measured against
Disfactory labels, §4); the incumbent is manual crowdsourcing + hand-written
...
#raw(block: true, "        SOURCES                 INGESTION                 LAKE / STORE
 Sentinel-2 L2A (STAC) ---> stac_fetch  (bi-temporal) ---> MinIO (S3): COGs, manifests
 Disfactory open API   ---> labels_fetch (eval set)   ---> PostGIS: parcels, labels,
 NLSC land-use/parcels ---> parcels_load                    detections, flagged, dossiers
 全國法規資料庫 statutes ---> laws_fetch -> chunk -> embed --> pgvector: law chunks
                                  |
                                  v
 PROCESSING:  change_detect (NDBI/NDVI; torchgeo-swappable)
              spark_tiles  (PySpark tiles the AOI, CD per tile)   -- batch -->
              zoning_join  (detections x farmland parcels, PostGIS ST_Intersects)
              evaluate     (precision / recall vs Disfactory labels)
                                  |
                                  v
 AGENT (value capture): dossier_agent
   RAG (pgvector) retrieves matching statute chunks + structured detection facts
   -> Claude drafts a grounded enforcement dossier -> adversarial draft->critic
                                  |
                                  v
 DELIVERY: FastAPI + Leaflet dashboard (flagged parcels, before/after imagery,
   downloadable dossiers, eval metrics). Kafka event: new detection -> worker
   -> dossiers -> live dashboard update (SSE).")
...
#table(
  ...
  [Apache Kafka (KRaft)], [Velocity: a `detection.completed` event fans out to a worker (join→eval→dossiers) and to the dashboard over SSE — new imagery flows to fresh dossiers with no manual rerun. Any Kafka-API broker (e.g. Redpanda) works.],
...
*Zoning join — "illegal" = change + zoning.* A change polygon is only a _candidate
violation_ once it sits on agricultural land. `zoning_join` intersects detections
with farmland parcels in PostGIS, keeps only farmland overlaps, attaches the 地號,
finds the nearest Disfactory label, and ranks by confidence. We never label a
parcel "illegal" without the zoning overlay.
...
factory (precisely the value: surfacing the unreported). A verified offline
demo run (synthetic bi-temporal imagery with injected building patches, deterministic
...
The score is deliberately non-trivial (two detected patches have no label →
false positives; one labelled factory has no detectable patch → a miss), so the
demo reports an honest figure rather than a staged 100%. The same metric is shown
live on the dashboard for any run; the live pipeline runs the identical stages on
a real Sentinel-2 pair for the AOI.
...
*Unit economics.* A dossier is a few thousand tokens of context + output ≈ a few
US cents per parcel, versus monitoring contracts in the hundreds of thousands of
NT\$ and volunteer-hours per detection. The compounding moat is the *imagery
...
- *Data acquisition cost & licensing.* Sentinel-2 and Disfactory are open
  ...
  so the demo ships a deterministic synthetic parcel layer (labelled
...
- *Competition & moat.* A scraper of Disfactory is worthless (the data is free); the
  moat is the fused per-parcel substrate, the inspector-judgment scaffolding encoded
  in the detection→zoning→dossier pipeline, the compounding archive, and
...
(unit + DB-integration, incl. zoning-join correctness). The repository contains
all ingestion / processing / agent / delivery code, a runnable `README.md`, and an
